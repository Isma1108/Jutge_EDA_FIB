...
bool pot_menjar(VVC& tauler, Pos p) {
	int i = p.i, j = p.j;
	if (tauler[i][j] == 'X') return false;
	if (tauler[i][j] == 'B') return true;
	tauler[i][j] = 'X';
	return pot_menjar(tauler, Pos(i+1,j)) or pot_menjar(tauler, Pos(i-1,j))
		or pot_menjar(tauler, Pos(i,j+1)) or pot_menjar(tauler, Pos(i, j-1));
}
...
int main() {
	...
	while (cin >> f >> c) {
		...
		//Poso a X aquelles caselles que implicaran adjacència amb un fantasma
		for (int i = 0; i < fantasmes.size(); ++i) {
			for (int k = 0; k < 8; ++k) 
				tauler[fantasmes[i].i + FI[k]][fantasmes[i].j + FJ[k]] = 'X';
		}

		if (pot_menjar(tauler, p_inicial)) cout << "si" << endl;
		else cout << "no" << endl;
	}
}
